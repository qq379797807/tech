package com.lty.wuzi3.chess;

import java.util.ArrayList;

import com.lty.wuzi3.chess.piece.ChessPiece;
...
public class ChessBoard {
    ...
    public int run(ChessPiece cp, int row, int coulnm) {
    ...
    public int run(Luozi entity) {

        ChessPiece cp = null;

        if (entity.getColor().equals( "black")) {
            cp = new ChessPieceBlack();
        } else if (entity.getColor().equals( "white")) {
            cp = new ChessPieceWhite();
        }

        // 解析 cellId（从 "2-5" 格式转换为 x 和 y 坐标）
        int row = Integer.parseInt(entity.getCellId().split("-")[0]);
        int column = Integer.parseInt(entity.getCellId().split("-")[1]);
